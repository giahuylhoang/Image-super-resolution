...
class MetaUpScale(Layer):
    ...
    def build(self, input_shape):
		...
		self.kernel_1 = self.add_weight(name='kernel_fc_1',
										shape=(self.hidden_units,self.kernel_size),
										initializer=self.kernel_initializer,
										regularizer=self.kernel_regularizer,
										constraint=self.kernel_constraint)
		self.kernel_1 = self.add_weight(name='kernel_fc_3',
										shape=(self.hidden_units,self.hidden_units),
										initializer=self.kernel_initializer,
										regularizer=self.kernel_regularizer,
										constraint=self.kernel_constraint)
		self.kernel_3 = self.add_weight(name='kernel_fc_3',
										shape=((self.kernel_size ** 2) * in_C * out_C,256),
										initializer=self.kernel_initializer,
										regularzier=self.kernel_regularizer,
										constraint=self.kernel_constraint)
	...
	def call(self, inputs):
		...
		features = inputs[0]
		SR_images = list()
		for feature, scale in zip(scales, features):
			...
			padd = int(self.kernel_size/2)
  			...
  			for i  in range(out_height):
    			for j in range(out_width):
					...
					v_tensor = tf.constant([v_i, v_j, 1 / scale])
      				i_prime = int(i / scale)
      				j_prime = int(j / scale)
					#Predict_weight
					weights = tf.add(tf.matmul(self.kernel_1, v_tensor), self.bias_1)
					weights = tf.nn.relu(weights)
					weights = tf.add(tf.matmul(self.kernel_2, weights), self.bias_2)
					weights = tf.nn.relu(weights)
					weights = tf.add(tf.matmul(self.kernel_3, weights), self.bias_3)
      				weights = tf.reshape(weights, [self.kernel_size ,self.kernel_size ,in_C , out_C ])
      				window = padded_feature[i_prime:i_prime + self.kernel_size, 
					  						j_prime:j_prime + self.kernel_size],
